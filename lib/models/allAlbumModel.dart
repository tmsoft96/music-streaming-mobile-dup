...
class AllAlbumModel {
  ...
  String? msg;
  int? count;
  List<AllAlbumData>? data;

  AllAlbumModel({this.ok, this.msg, this.count, this.data});

  AllAlbumModel.fromJson({
    ...
    if (json != null) {
      ...
      count = json['count'];
      if (json['data'] != null) {
        List<AllAlbumData> data = <AllAlbumData>[];
        json['data'].forEach((v) {
          data.add(new AllAlbumData.fromJson(v));
        });
        this.data = <AllAlbumData>[];
        if (filterByArtistId != null)
          data.forEach((AllAlbumData d) {
...
class AllAlbumData {
  ...
  AllAlbumData.fromJson(Map<String, dynamic> json) {
    id = json['id'];
