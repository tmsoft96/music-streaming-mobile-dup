...
import 'widget/todaysHitsFullWidget.dart';
import 'widget/todaysHitsWidget.dart';
...
class _TodaysHitsState extends State<TodaysHits> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: allMusicTodaysHitsModelStream,
      initialData: allMusicTodaysHitsModel ?? null,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          if (snapshot.data.ok)
            return _mainContent(snapshot.data);
          else
            return allMusicTodaysHitsModel == null
                ? widget.noOfContentDisplay == -1000
                    ? emptyBox(context, msg: "${snapshot.data.msg}")
                    : emptyBoxLinear(context, msg: "${snapshot.data.msg}")
                : _mainContent(allMusicTodaysHitsModel!);
  ...
  Widget _mainContent(AllMusicTodaysHitsModel model) {
    ...
    return widget.noOfContentDisplay == -1000
        ? todaysHitsFullWidget(
            context: context,
            onMusic: (MusicData data) => _onSong(data, model),
            model: model,
        ...
        : todaysHitsWidget(
            context: context,
            onSeeAll: () => _onSeeAll(),
            onSong: (MusicData data) => _onSong(data, model),
            fromAdminPage: widget.fromAdminPage,
            model: model,
            selectedGenre: widget.selectedGenre,
            onSongMore: (MusicData data) => _onTrackMore(data, model),
            onSongPlay: (MusicData data) => _onPlayTrack(data, model),
          );
  }

  void _onPlayTrack(MusicData data, AllMusicTodaysHitsModel model) {
  ...
  void _onTrackMore(MusicData data, AllMusicTodaysHitsModel model) {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => Scaffold(
          appBar: AppBar(title: Text("Today's Hit")),
          body: TodaysHits(
  ...
  Map<String, dynamic> _getJson(MusicData data, AllMusicTodaysHitsModel model) {
  ...
  void _onSong(MusicData data, AllMusicTodaysHitsModel model) {
