...
import 'widget/topPicksMusicMusicFullWidget.dart';
import 'widget/topPicksMusicWidget.dart';

class TopPicksMusic extends StatefulWidget {
  ...
  @override
  State<TopPicksMusic> createState() => _TopPicksMusicState();
}

class _TopPicksMusicState extends State<TopPicksMusic> {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: allMusicTopPicksModelStream,
      initialData: allMusicTopPicksModel ?? null,
      builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
        if (snapshot.hasData) {
          if (snapshot.data.ok)
            return _mainContent(snapshot.data);
          else
            return allMusicTopPicksModel == null
                ? widget.noOfContentDisplay == -1000
                    ? emptyBox(context, msg: "${snapshot.data.msg}")
                    : emptyBoxLinear(context, msg: "${snapshot.data.msg}")
                : _mainContent(allMusicTopPicksModel!);
  ...
  Widget _mainContent(AllMusicTopPicksModel model) {
  ...
  void _onPlayTrack(MusicData data, AllMusicTopPicksModel model) {
  ...
  void _onTrackMore(MusicData data, AllMusicTopPicksModel model) {
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => Scaffold(
          appBar: AppBar(title: Text("Top Picks")),
          body: TopPicksMusic(
  ...
  Map<String, dynamic> _getJson(MusicData data, AllMusicTopPicksModel model) {
  ...
  void _onMusic(MusicData data, AllMusicTopPicksModel model) {
